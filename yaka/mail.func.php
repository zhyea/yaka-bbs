...
class PHPMailer
{
    ...
    public function WrapText($message, $length, $qp_mode = false)
    {
        ...
        for ($i = 0; $i < count($line); $i++) {
            ...
            for ($e = 0; $e < count($line_part); $e++) {
                $word = $line_part[$e];
                if ($qp_mode and (str_length($word) > $length)) {
                    $space_left = $length - str_length($buf) - 1;
                    if ($e != 0) {
                        if ($space_left > 20) {
                            $len = $space_left;
                            if ($is_utf8) {
                    ...
                    if (str_length($buf) > $length and $buf_o != '') {
    ...
    public function EncodeString($str, $encoding = 'base64')
    {
        ...
        switch (strtolower($encoding)) {
            ...
            case '8bit':
                ...
                if (substr($encoded, -(str_length($this->LE))) != $this->LE)
                    $encoded .= $this->LE;
                break;
            case 'binary':
                $encoded = $str;
    ...
    public function EncodeHeader($str, $position = 'text', $pl = 0)
    {
        ...
        $maxlen = 75 - 7 - str_length($this->CharSet);
        // Try to select the encoding which should produce the shortest output
        if (str_length($str) / 3 < $x) {
            $encoding = 'B';
    ...
    public function HasMultiBytes($str)
    {
        if (function_exists('mb_strlen')) {
            return (str_length($str) > mb_strlen($str, $this->CharSet));
    ...
    public function Base64EncodeWrapMB($str)
    {
        ...
        $length = 75 - str_length($start) - str_length($end);
        // Average multi-byte ratio
        $ratio = $mb_length / str_length($str);
        ...
        for ($i = 0; $i < $mb_length; $i += $offset) {
            ...
            do {
                ...
                $lookBack++;
            } while (str_length($chunk) > $length);

            $encoded .= $chunk . $this->LE;
        }

        // Chomp the last linefeed
        $encoded = substr($encoded, 0, -str_length($this->LE));
        return $encoded;
    }
    ...
    public function EncodeQPphp($input = '', $line_max = 76, $space_conv = false)
    {
        ...
        foreach ($lines as $line) {
            ...
            for ($i = 0; $i < $linlen; $i++) {
                ...
                if ((str_length($newline) + str_length($c)) >= $line_max) { // CRLF is not counted
    ...
    {
        return str_length($this->AltBody) > 0;
    }
        ...
        if (isset($images[2])) {
            foreach ($images[2] as $i => $url) {
                ...
                if (!preg_match('#^[A-z]+://#', $url)) {
                    ...
                    if (str_length($basedir) > 1 && substr($basedir, -1) != '/') {
                        $basedir .= '/';
                    }
                    if (str_length($directory) > 1 && substr($directory, -1) != '/') {
                        $directory .= '/';
                    }
    ...
    public function DKIM_BodyC($body)
    {
        ...
        while (substr($body, str_length($body) - 4, 4) == "\r\n\r\n") {
            $body = substr($body, 0, str_length($body) - 2);
        }
        return $body;
    }
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
        ...
        $DKIMlen = str_length($body); // Length of body
...
class SMTP
{
     ...
     */
        ...
        foreach ($lines as $line) {
            ...
            while (str_length($line) > $max_line_length) {
